...
import { diagnosticQuestions, diagnosticResults, formatDiagnosticShareText, getDominantDiagnostic } from "../client/src/data/creatorDiagnostic";

const pageSource = readFileSync(resolve(import.meta.dirname, "..", "client/src/pages/CreatorDiagnostic.tsx"), "utf8");
...
describe("療癒型創作者內容承接診斷", () => {
  it("provides four actionable questions across positioning, content, and conversion", () => {
    ...
    expect(diagnosticQuestions.every((question) => question.options.length === 3)).toBe(true);
    expect(Object.keys(diagnosticResults)).toEqual(["positioning", "content", "conversion"]);
  });

  it("returns the dominant diagnosis and supplies both tool and LINE next steps", () => {
    expect(getDominantDiagnostic(["content", "content", "positioning", "conversion"]).key).toBe("content");
  ...
  it("labels the homepage evidence section as real supplied student case material", () => {
    ...
    expect(homeSource).toContain("Vivi 已提供的實際案例截圖與成果摘要");
