...
describe("tarot reading input", () => {
  it("accepts a known card and focus", () => {
    expect(
      tarotReadingInput.parse({ cardId: "star", focus: "今日狀態" })
    ).toEqual({ cardId: "star", focus: "今日狀態" });
  });

  it("rejects unknown card ids and focuses", () => {
    expect(() =>
      tarotReadingInput.parse({ cardId: "unknown", focus: "今日狀態" })
    ).toThrow();
    expect(() =>
      tarotReadingInput.parse({ cardId: "star", focus: "未定義主題" })
    ).toThrow();
  });

  it("accepts three unique cards for a past-present-future spread and rejects duplicate cards", () => {
    expect(
      tarotSpreadInput.parse({
        cardIds: ["star", "moon", "sun"],
        focus: "自我照顧",
      })
    ).toEqual({ cardIds: ["star", "moon", "sun"], focus: "自我照顧" });
    expect(() =>
      tarotSpreadInput.parse({
        cardIds: ["star", "star", "sun"],
        focus: "自我照顧",
      })
    ).toThrow();
  });
  ...
  it("parses a structured three-card response and preserves a non-predictive fallback", () => {
    ...
    expect(
      parseTarotSpreadReading(`\`\`\`json\n${json}\n\`\`\``, cards, "今日狀態")
        .synthesis
    ).toBe("整合");
    expect(
      parseTarotSpreadReading("請慢慢整理感受。", cards, "今日狀態").future
    ).toContain("不是結果保證");
  });
  ...
  it("maps every selectable major arcana card to a classic visual and keeps the brand card back", () => {
    ...
    expect(styles).toMatch(
      /\.tarot-card-3d\.is-revealed\s*{[^}]*transform:\s*rotateY\(180deg\);/s
    );
