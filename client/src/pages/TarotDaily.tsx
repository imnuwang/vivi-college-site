...
function useLocalMutation<TInput, TOutput>(buildResult: (input: TInput) => TOutput) {
...
function buildSingleReading({ cardId, focus }: { cardId: TarotCardId; focus: TarotFocus }) {
  ...
  return {
    ...
    reading: {
      ...
      action: "留 10 分鐘，把現在最想處理的事縮成一步。這一步要小到今天就能完成。",
      journalPrompt: `面對「${focus}」，如果今天只能回應一個需要，我會先照顧哪一件事？`,
      boundary: "這是一則自我覺察提示，不預言未來，也不取代醫療、法律、財務或其他重大決策所需的專業意見。",
    },
  };
}

function buildSpreadReading({ cardIds, focus }: { cardIds: TarotCardId[]; focus: TarotFocus }) {
  const cards = cardIds.map(cardId => tarotCards.find(card => card.id === cardId)).filter((card): card is TarotCard => Boolean(card));
...
export default function TarotDaily() {
  ...
  return <>
    <Seo title="塔羅一日一牌：把今日訊號整理成一個小行動" description="抽一張今日塔羅牌，得到一則自我覺察、書寫問題與 10 分鐘行動提示。僅作為反思，不提供未來預言或重大決策建議。" path="/tarot-daily" schema={{ "@context": "https://schema.org", "@type": "WebApplication", name: "美心學苑塔羅一日一牌", applicationCategory: "LifestyleApplication", inLanguage: "zh-TW", isAccessibleForFree: true }} />
    <PageIntro kind="workbook" chapter="05" eyebrow="ONE CARD · ONE PAUSE" title={<>今天不需要知道全部。<br /><em>先聽一張牌提醒你的事。</em></>} description="抽牌不是找一個替你決定的答案，而是替當下留出一段停頓：看見感受、寫下一句話，然後選一個你今天做得到的小行動。" note="牌卡內容僅供自我覺察與書寫提示，不預言未來，也不取代專業意見。" />
    ...
    {(reading.isPending || spreadReading.isPending) && <section className="pb-16"><div className="site-shell"><div className="tarot-reading-card animate-pulse"><p className="section-eyebrow">PAUSE BEFORE READING</p><div className="mt-5 h-9 w-2/3 rounded bg-[#e5e8df]" /><div className="mt-5 h-4 w-full rounded bg-[#e5e8df]" /><div className="mt-3 h-4 w-4/5 rounded bg-[#e5e8df]" /></div></div></section>}
    ...
    {spreadData && <section className="pb-20"><div className="site-shell tarot-reading-card tarot-spread-reading"><SectionEyebrow>過去、現在、未來 · {spreadData.focus}</SectionEyebrow><h2 className="display-heading mt-4">{spreadData.reading.opening}</h2><div className="tarot-spread-reading-grid">{spreadData.cards.map((spreadCard, index) => <article key={spreadCard.id}><span>{tarotSpreadPositions[index]!.label} · {spreadCard.name}</span><h3>{spreadCard.keyword}</h3><p>{index === 0 ? spreadData.reading.past : index === 1 ? spreadData.reading.present : spreadData.reading.future}</p></article>)}<article className="tarot-spread-synthesis"><span>三張牌一起說的事</span><h3>從回看，走向選擇</h3><p>{spreadData.reading.synthesis}</p></article><article className="tarot-spread-synthesis"><span>下一步與書寫</span><h3>{spreadData.reading.action}</h3><p>{spreadData.reading.journalPrompt}</p></article></div><p className="tarot-boundary"><Compass className="size-4" />{spreadData.reading.boundary}</p></div></section>}
    {(reading.isError || spreadReading.isError) && <section className="pb-20"><div className="site-shell"><div className="tarot-reading-card"><h2 className="font-serif text-2xl text-[#183b31]">這一刻暫時無法整理提示。</h2><p className="mt-3 text-sm leading-7 text-[#627167]">你仍可以把抽到的牌當作今天的書寫提示：它讓你想到哪一段經驗？這個感受今天最需要什麼？</p></div></div></section>}
