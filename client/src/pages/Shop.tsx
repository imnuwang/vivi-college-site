    ...
    <>
      ...
      <section className="section-space pt-10">
        ...
        <div className="site-shell mt-10 shop-grid">
          {products.map((product) => <article className="product-card" key={product.id}><div className="product-image-wrap" style={{ backgroundImage: `url(${product.image ?? asset.hero})`, backgroundPosition: "center", backgroundSize: "cover" }}><img src={product.image} alt={product.name} /></div><div className="product-card-copy"><p>{product.category}</p><h2>{product.name}</h2><span className="product-price">{product.priceLabel}</span><p className="product-description">{product.description}</p><div className="product-detail"><Check className="size-4" />{product.detail}</div><p className="mt-4 text-sm leading-6 text-[#59675f]">線上付款，付款完成後即交付數位檔案。</p><a href={product.paymentUrl} target="_blank" rel="noreferrer" className="vivi-button vivi-button-dark mt-6 w-full">前往付款 <CreditCard className="size-4" /></a></div></article>)}
        </div>
      </section>
